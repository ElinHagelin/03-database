...
const db = await getData('bookreview.json', [])
...
async function bookReview() {
	...
	if (pick === '1') {
		...
		let rating = await question('\nPlease rate the book on a scale of 1-5:2 ')
		...
		db.data.forEach(({ title, author, rating }) => {
		...
		db.data.splice(db.data.indexOf(db.data.find(review => review.id == remove)), 1)
		...
		db.data.find(review => review.id == change).rating = newRating
